...
Kho lưu trữ này được tích hợp sẵn cấu hình **Jupyter Book** để chuyển đổi toàn bộ mã nguồn thành một website học liệu trực quan. Để tự build sách HTML trên máy của bạn, hãy chạy lệnh `jupyter-book build .` trong terminal để biên dịch toàn bộ dự án thành định dạng HTML:

```bash
# Di chuyển vào thư mục dự án
cd amsa_book

# Biên dịch toàn bộ dự án thành định dạng HTML
jupyter-book build .
```

Sau khi quá trình build kết thúc thành công, bạn chỉ cần mở file `_build/html/index.html` trên trình duyệt web để bắt đầu đọc sách!

## 🌐 Xuất bản lên GitHub Pages

Hãy chạy lệnh git để commit toàn bộ thay đổi mới nhất. Sau đó, sử dụng thư viện ghp-import (lệnh: `ghp-import -n -p -f _build/html`) để đẩy thư mục HTML này lên nhánh `gh-pages` trên kho lưu trữ GitHub của dự án:
...
# Đẩy thư mục HTML lên nhánh gh-pages
